...
public abstract class World implements IBlockAccess
{
    ...
    public List<BlockPos> getCollidingBlockPositions(Entity entityIn, AxisAlignedBB bb) { //TODO Jigsaw method
		...
		for (int loop0 = minX; loop0 < maxX; ++loop0) {
			for (int loop1 = minZ; loop1 < maxZ; ++loop1) {
				if (this.isBlockLoaded(blockPos.setPos(loop0, 64, loop1))) {
					for (int loop2 = minY - 1; loop2 < maxY; ++loop2) {
						blockPos.setPos(loop0, loop2, loop1);
